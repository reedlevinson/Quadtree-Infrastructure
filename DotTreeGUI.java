...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			// TODO: YOUR CODE HERE
			// if tree is empty makes a new tree with this dot as the root
			if (tree == null) tree = new PointQuadtree<>(new Dot(x, y), 0, 0, width, height);
			// else inserts new dot into tree
	...
	private void test2() {
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(300,400), 0,0,800,600); // start with A
		tree.insert(new Dot(100,150)); // B
		tree.insert(new Dot(450,550)); // C
		tree.insert(new Dot(250,350)); // D
		tree.insert(new Dot(150,200)); // E
		tree.insert(new Dot(650, 550)); // F
		int bad = 0;
		bad += testFind(0,0,900,6,6,6); // rect for all; circle for all; find all
		bad += testFind(275,375,100,6,5,2); // rect for all; circle for all; find A, D
		bad += testFind(125, 175, 50, 5, 4, 2); // rect for A [B [D [E]]] [C]; circle for A, B, D, E; find B, E
		bad += testFind(660, 560, 20, 4, 3, 1); // rect for A [B] [C [F]]; circle for A, C, F; find F
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// TODO: YOUR CODE HERE
		g.fillOval((int) tree.pointX() - dotRadius, (int) tree.pointY() - dotRadius, 2*dotRadius, 2*dotRadius);
		g.drawLine(tree.getX1(), (int) tree.pointY(), tree.getX2(), (int) tree.pointY());
		g.drawLine((int) tree.pointX(), tree.getY1(), (int) tree.pointX(), tree.getY2());
